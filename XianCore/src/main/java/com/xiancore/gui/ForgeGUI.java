...
import com.xiancore.systems.forge.ForgeDisplayService;
import com.xiancore.systems.forge.ForgeDisplayService.ForgeStatusInfo;
...
public class ForgeGUI {
    ...
    public ForgeGUI(XianCore plugin) {
        this.plugin = plugin;
        this.displayService = new ForgeDisplayService(plugin);
    }
    ...
    private void show(Player player) {
        ...
        ForgeStatusInfo status = displayService.getForgeStatus(player);

        displayPlayerStatus(status, contentPane);
        displayMakeButton(contentPane);
        displayCraftButton(contentPane);
        displayRefineButton(status, contentPane);
        displayEnhanceButton(status, contentPane);
        displayFuseButton(player, status, contentPane);
        displayQuickActions(contentPane);
        displayHelpButton(contentPane);
     ...
     * 显示玩家状态（第0行）
     */
    private void displayPlayerStatus(ForgeStatusInfo status, StaticPane pane) {
        // 灵石信息
        ItemStack spiritStoneItem = new ItemBuilder(Material.EMERALD)
                .name("§a§l灵石")
                .lore(
                        "§e当前灵石: §f" + status.getSpiritStones(),
                        "",
                        "§7炼制装备消耗灵石",
                        "§7品质越高消耗越多"
                )
        ...
        ItemStack embryoItem = new ItemBuilder(Material.ENDER_PEARL)
                .name("§d§l我的胚胎")
                .lore(
                        "§e背包中胚胎: §f" + status.getEmbryoCount() + " 个",
                        "",
                        "§7胚胎可以精炼成装备",
                        "§7使用 §e精炼装备 §7功能"
                )
        ...
        ItemStack equipmentItem = new ItemBuilder(Material.DIAMOND_SWORD)
                .name("§b§l我的装备")
                .lore(
                        "§e背包中装备: §f" + status.getEquipmentCount() + " 件",
                        "",
                        "§7可以强化或融合装备",
                        "§7提升装备属性"
                )
     ...
     * 配方炼制按钮（第1行，主要功能）
     */
    private void displayMakeButton(StaticPane pane) {
        ...
        pane.addItem(new GuiItem(makeButton, event -> {
            ...
            plugin.getServer().getScheduler().runTask(plugin, () -> {
                CraftingGUI.open((Player) event.getWhoClicked(), plugin);
            });
        }), 4, 1);
    }
     ...
     * 炼制胚胎按钮（第2行）
     */
    private void displayCraftButton(StaticPane pane) {
     ...
     * 精炼装备按钮（第2行）
     */
    private void displayRefineButton(ForgeStatusInfo status, StaticPane pane) {
        Material material = status.hasEmbryos() ? Material.SMITHING_TABLE : Material.BARRIER;
        String nameColor = status.hasEmbryos() ? "§a§l" : "§7";
        ...
        ItemStack refineButton = new ItemBuilder(material)
                ...
                .lore(
                        ...
                        "§e饰品 §7- 戒指/项链/法宝",
                        "",
                        status.hasEmbryos() ? "§a§l点击打开精炼界面" : "§7背包中没有胚胎"
                )
                .build();
     ...
     * 强化装备按钮（第2行）
     */
    private void displayEnhanceButton(ForgeStatusInfo status, StaticPane pane) {
        Material material = status.hasEquipments() ? Material.ENCHANTING_TABLE : Material.BARRIER;
        String nameColor = status.hasEquipments() ? "§b§l" : "§7";
        ...
        ItemStack enhanceButton = new ItemBuilder(material)
                ...
                .lore(
                        ...
                        "§7- 失败惩罚: §c10%概率降级",
                        "",
                        status.hasEquipments() ? "§a§l点击打开强化界面" : "§7背包中没有装备"
                )
                .build();
     ...
     * 融合装备按钮（第2行）
     */
    private void displayFuseButton(Player player, ForgeStatusInfo status, StaticPane pane) {
        String nameColor = status.canFuse() ? "§d§l" : "§7";
        ...
        ItemStack fuseButton = new ItemBuilder(Material.BREWING_STAND)
                ...
                .lore(
                        ...
                        "",
                        "§e前置条件:",
                        displayService.hasMainHandEmbryo(player) ? "§a✓ 主手持胚胎" : "§c✗ 主手需要胚胎",
                        displayService.hasOffHandEmbryo(player) ? "§a✓ 副手持胚胎" : "§c✗ 副手需要胚胎",
                        "",
                        "§e融合机制:",
                        "§7- 品质必须相同",
                        "§7- 20%概率提升品质",
                        "§7- 活跃灵气可提升概率",
                        "",
                        status.canFuse() ? "§a点击执行融合" : "§7请先准备好胚胎"
                )
                .build();
     ...
     * 显示快捷入口（第3行）
     */
    private void displayQuickActions(StaticPane pane) {
        // 配方图鉴按钮
     ...
     * 帮助按钮（第4行）
     */
    private void displayHelpButton(StaticPane pane) {
